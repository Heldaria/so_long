...
/*   Updated: 2022/05/16 20:26:56 by llepiney         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_up(t_solong *s)
{
	if (s->map[s->x - 1][s->y] != '1' && s->map[s->x - 1][s->y] != 'E')
	{
		...
		mlx_put_image_to_window(s->mlx_ptr, s->mlx_win, s->buck[3],
			s->y * 32, (s->x - 1) * 32);
...
void	move_left(t_solong *s)
{
	if (s->map[s->x][s->y - 1] != '1' && s->map[s->x][s->y - 1] != 'E')
	{
		...
		mlx_put_image_to_window(s->mlx_ptr, s->mlx_win, s->buck[6],
			(s->y - 1) * 32, s->x * 32);
		mlx_put_image_to_window(s->mlx_ptr, s->mlx_win, s->grass,
			s->y * 32, s->x * 32);
		s->y -= 1;
...
void	move_right(t_solong *s)
{
	if (s->map[s->x][s->y + 1] != '1' && s->map[s->x][s->y + 1] != 'E')
	{
		...
		mlx_put_image_to_window(s->mlx_ptr, s->mlx_win, s->buck[9],
			(s->y + 1) * 32, s->x * 32);
		mlx_put_image_to_window(s->mlx_ptr, s->mlx_win, s->grass,
			s->y * 32, s->x * 32);
		s->y += 1;
